...
import com.mudxx.captcha.core.domain.vo.PointVO;
...
import com.mudxx.common.utils.random.RandomUtils;
...
public class ClickWordCaptchaServiceImpl extends AbstractCaptchaService {
    ...
    protected String clickWordFontStr;
    /**
     * 点选文字字体
     */
    ...
    public String captchaType() {
        return CaptchaTypeEnum.CLICK_WORD.getCode();
    }

    @Override
    public void init(Properties config) {
        super.init(config);
        this.clickWordFontStr = config.getProperty(Const.CAPTCHA_FONT_TYPE, "WenQuanZhengHei.ttf");
        this.clickWordFont = FontUtils.loadFont(clickWordFontStr, HAN_ZI_SIZE);
    ...
    public CaptchaVO get(CaptchaGetDTO getDTO) {
        super.preGetLimit(getDTO.getBrowserInfo());
        BufferedImage bufferedImage = ImageUtils.getPicClick();
        if (null == bufferedImage) {
            throw new BizException(CaptchaErrorCode.CAPTCHA_BASEMAP_NULL);
        }
        CaptchaVO imageData = getImageData(bufferedImage);
        if (StrUtil.isBlank(imageData.getOriginalImageBase64())) {
            throw new BizException(CaptchaErrorCode.CAPTCHA_GET_ERROR);
        }
        return imageData;
    }
    ...
    public CaptchaVO check(CaptchaCheckDTO checkDTO) {
        super.preCheckLimit(checkDTO.getBrowserInfo());
        //取坐标信息
        String codeKey = String.format(REDIS_CAPTCHA_KEY, checkDTO.getToken());
         ...
         */
        try {
            point = JSONUtil.toList(s, PointVO.class);
            //aes解密
            pointJson = decrypt(checkDTO.getPointJson(), point.get(0).getSecretKey());
            point1 = JSONUtil.toList(pointJson, PointVO.class);
        } catch (Exception e) {
        ...
        String value = null;
        try {
            value = AESUtil.aesEncrypt(checkDTO.getToken().concat("---").concat(pointJson), secretKey);
        } catch (Exception e) {
    ...
    private CaptchaVO getImageData(BufferedImage backgroundImage) {
        ...
        CaptchaServiceFactory.getCache(cacheType).set(codeKey, JSONUtil.toJsonStr(pointList), EXPIRES_SECONDS);
