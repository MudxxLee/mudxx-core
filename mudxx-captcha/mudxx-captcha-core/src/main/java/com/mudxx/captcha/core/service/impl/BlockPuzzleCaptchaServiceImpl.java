...
public class BlockPuzzleCaptchaServiceImpl extends AbstractCaptchaService {
    ...
    public String captchaType() {
        return CaptchaTypeEnum.BLOCK_PUZZLE.getCode();
    }
    ...
    public CaptchaVO check(CaptchaCheckDTO checkDTO) {
        ...
        try {
            point = JSONUtil.toBean(s, PointVO.class);
            //aes解密
            pointJson = decrypt(checkDTO.getPointJson(), point.getSecretKey());
            point1 = JSONUtil.toBean(pointJson, PointVO.class);
        } catch (Exception e) {
...
//            base64StrToImage(encoder.encodeToString(jigsawImages), "D:\\滑动.png");
            ...
            CaptchaServiceFactory.getCache(cacheType).set(codeKey, JSONUtil.toJsonStr(point), EXPIRES_SECONDS);
            logger.debug("token：{},point:{}", dataVO.getToken(), JSONUtil.toJsonStr(point));
            return dataVO;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
